...
#pragma warning(pop)
	...
	template <typename T>
	inline CString ToString(const Vec4<T>& a)
	{
		CString result;
...
#pragma warning(pop)
	...
	template <typename T>
	inline CString ToString(const Mat2<T>& a)
	{
		CString result;
	...
	template <typename T>
	inline CString ToString(const Mat3<T>& a)
	{
		CString result;
		result.Add('(');
	...
	//template <typename T>
	//inline CString ToString(const Mat4<T>& a)
	//{
	//	CString result;
	//	result.Add('(');
	//	result.Add(ToString(a.row0));
	...
	public:
		...
		{
			orientation = cm::GlobalRotateX(rads, orientation);
		}

		inline void GlobalRotateY(const real32& rads)
		{
			orientation = cm::GlobalRotateY(rads, orientation);
		}
		...
		{
			Mat4f result = cm::CalculateTransformMatrix(this->position, this->scale, this->orientation);
