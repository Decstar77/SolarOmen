...
namespace sol
{
	...
	CubeMapInstance CubeMapInstance::Create(uint32 resolution)
	{
		...
		for (int32 i = 0; i < 6; i++)
		{
			...
			DXCHECK(dc.device->CreateRenderTargetView(result.texture, &render_target_desc, &result.renderFaces[i]));
		}
	...
	void CubeMapInstance::Release(CubeMapInstance* cube)
	{
		DXRELEASE(cube->renderFaces[0]);
		DXRELEASE(cube->renderFaces[1]);
		DXRELEASE(cube->renderFaces[2]);
		DXRELEASE(cube->renderFaces[3]);
		DXRELEASE(cube->renderFaces[4]);
		DXRELEASE(cube->renderFaces[5]);

		DXRELEASE(cube->shaderView);
		DXRELEASE(cube->texture);
	}
