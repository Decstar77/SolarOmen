...
namespace cm
{
	...
	std::vector<Texture> TextureProcessor::LoadTextures(const std::vector<CString>& texturePaths, const MetaProcessor& metaProcessor)
	{
		...
		{
			CString texturePath = texturePaths.at(i);
			CString metaPath = metaProcessor.Find(textureNames.at(i));
...
#include "../vendor/std_image/std_image.h"
	...
	void Texture::LoadTexture(const CString& path)
	{
		int32 width = -1;
		int32 height = -1;
		int32 channels = -1;
